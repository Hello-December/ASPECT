...
class UKF:
    ...
    def __init__(
        ...
        #adaptive parameters
        # -------------------------
        # Adaptive Q (NIS-based scaling)
        # Q_max ~ 1e-5, Q_min ~ 1e-8  => scale in [1e-3, 1]
        # -------------------------
        self.Q0 = self.Q.copy()      # 基准Q（你外面给的Q0）
        self.Q_scale = 1.0           # Q缩放因子
        self.adapt_Q = True
        self.adapt_eta_Q = 0.02      # 调整速度（建议0.005~0.02）
        self.Q_scale_min = 1e-4      # => origin * 1e-3
        self.Q_scale_max = 1.0       # => origin
    ...
    def _sigma_points(self, x: np.ndarray, P: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            S = np.linalg.cholesky(P + jitter * np.eye(self.n))
    ...
    def update(self, z: np.ndarray, hx_args=()):
        ...
        # NIS = y^T S^{-1} y, 期望大约是 m
        if getattr(self, "adapt_Q", False):
            nis = float(y.T @ np.linalg.solve(Pzz, y))
            target = self.m  # E[NIS] ~ m

            ratio = nis / max(target, 1e-12)
            # ratio>1 -> 放大Q；ratio<1 -> 缩小Q
            self.Q_scale *= np.exp(self.adapt_eta_Q * (ratio - 1.0))
            self.Q_scale = float(np.clip(self.Q_scale, self.Q_scale_min, self.Q_scale_max))
...
def ukf_infer():
    # 替换所有 \ 为 /，支持跨平台，无转义错误
    pred_poses_path = r"./files/rot_speed_analysis/test_refiner_v1.1_train_3_random_light_0.1_40_0.08_0.1_1000_v2_2000imgs_v12/pred_rot_euler_cache.npy"
    gt_poses_path = r"./files/predefined_poses/0.1_40_0.08_0.1_1000_v2.npy"
    ...
    for i in range(pred_poses.shape[0]):
        ...
        q_scales.append(my_ukf.Q_scale)
    ...
    project_root = "./"
    output_dir = os.path.join(project_root, "files", "ukf_result", "dynamic_Q", subdirectory_name)
    ...
    plt.ylabel("Q_scale")
    plt.title("Adaptive Q_scale over time")
    ...
    print(f"  - Q_scale plot: {qscale_plot_path}")
